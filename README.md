...
本项目基于本地媒体目录进行扫描，不仅支持常见图片格式，还支持 iOS 实况照片 (`.livp`) 以及多种常规视频格式 (`.mp4`, `.mov`, `.avi`, `.mkv`) 的智能抽帧提取。通过接入图片分析模型（如 `GLM-4.1V-9B-Thinking`）理解媒体核心画面、场景及包含的物体信息，并存入本地 SQLite 数据库。同时支持通过自然语言进行相册搜索，利用文本处理模型（如 `Qwen2.5-7B-Instruct`）提供智能辅助重排序与更精准的找图能力。

## 预置模型与服务

默认使用 **SiliconFlow (硅基流动)** 的 API 进行图片和文本信息的双重模型推理处理：

- **默认图片模型**：`THUDM/GLM-4.1V-9B-Thinking`（主要用于扫描和理解图意）
- **默认文本处理模型**：`Qwen/Qwen2.5-7B-Instruct`（主要用于搜索、同义词扩展和重排序）
