"""
=============================================================================
LIVP 原地解压工具 (机械硬盘极致优化版)
=============================================================================

【脚本描述】
本脚本用于批量处理 iPhone 导出的 .livp 文件（Live Photo 压缩包）。
它会将 .livp 文件解压，提取出其中的静态图片（.jpg/.heic）和动态视频（.mov），
并将它们放置在与原文件同名的新文件夹中。
处理完成后，原 .livp 文件会被清空（变为 0KB），但会保留原始的修改时间戳，
以达到“释放磁盘空间且标记为已处理”的目的，同时避免同步软件产生混乱。

【核心优化：针对机械硬盘 (HDD)】
传统的“边读边解压边写”模式会导致机械硬盘的磁头在不同扇区之间疯狂寻道（Random I/O），
极大拖慢处理速度。本脚本采用了“大批次内存吞吐”策略：
    1. 连续读取：将一批文件（如 200 个）一次性顺序读取到内存中。
    2. 内存计算：完全在 RAM 中进行 ZIP 解压和二进制提取，不占用任何磁盘 I/O。
    3. 连续写入：将处理好的数据集中、顺序地写回磁盘。
通过将随机 I/O 转化为顺序 I/O，大幅提升了在机械硬盘上的处理效率。

【配置说明】
- SOURCE_DIR: 包含 .livp 文件的根目录路径。
- BATCH_SIZE: 每一批次处理的文件数量。
  * 建议值：50-200。数值越大，占用的内存越多，但对硬盘越友好。
  * 参考占用：BATCH_SIZE=200 时，约占用 12GB 内存。

【破坏性警告】
本脚本会**清空原有的 .livp 文件内容（变为0KB）**。在首次大批量运行前，
请务必确认已经做好了数据备份，或先使用小批量数据进行测试！
=============================================================================
"""
